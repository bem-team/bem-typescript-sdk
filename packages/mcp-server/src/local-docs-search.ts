...
const EMBEDDED_METHODS: MethodEntry[] = [
  {
    ...
    params: [
      "create_function: { functionName: string; type: 'extract'; displayName?: string; enableBoundingBoxes?: boolean; outputSchema?: object; outputSchemaName?: string; preCount?: boolean; tabularChunkingEnabled?: boolean; tags?: string[]; } | { functionName: string; type: 'classify'; classifications?: { name: string; description?: string; functionID?: string; functionName?: string; isErrorFallback?: boolean; origin?: object; regex?: object; }[]; description?: string; displayName?: string; nativeVisualInput?: boolean; tags?: string[]; } | { functionName: string; type: 'send'; destinationType?: 'webhook' | 's3' | 'google_drive'; displayName?: string; googleDriveFolderId?: string; s3Bucket?: string; s3Prefix?: string; tags?: string[]; webhookSigningEnabled?: boolean; webhookUrl?: string; } | { functionName: string; type: 'split'; displayName?: string; printPageSplitConfig?: { nextFunctionID?: string; nextFunctionName?: string; }; semanticPageSplitConfig?: { itemClasses?: object[]; }; splitType?: 'print_page' | 'semantic_page'; tags?: string[]; } | { functionName: string; type: 'join'; description?: string; displayName?: string; joinType?: 'standard'; outputSchema?: object; outputSchemaName?: string; tags?: string[]; } | { functionName: string; type: 'payload_shaping'; displayName?: string; shapingSchema?: string; tags?: string[]; } | { functionName: string; type: 'enrich'; config?: { steps: enrich_step[]; endpoints?: object[]; }; displayName?: string; tags?: string[]; } | { functionName: string; type: 'parse'; displayName?: string; extraConfig?: { enableBoundingBoxes?: boolean; }; parseConfig?: { defaultBucket?: string; extractEntities?: boolean; linkAcrossDocuments?: boolean; schema?: object; }; tags?: string[]; } | { functionName: string; renderConfig: { template: object; }; type: 'render'; displayName?: string; tags?: string[]; };",
    ],
    response:
      '{ function: object | object | object | object | object | object | object | object | object | object | object; }',
    perLanguage: {
      cli: {
        method: 'functions create',
        example:
          "bem functions create \\\n  --api-key 'My API Key' \\\n  --function-name functionName \\\n  --type extract \\\n  --render-config '{template: {base64: base64}}'",
      },
      csharp: {
        method: 'Functions.Create',
        example:
  ...
  },
  {
    ...
    perLanguage: {
      ...
      go: {
        ...
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/bem-team/bem-go-sdk"\n\t"github.com/bem-team/bem-go-sdk/option"\n)\n\nfunc main() {\n\tclient := bem.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t)\n\tfunctionResponse, err := client.Functions.Get(\n\t\tcontext.TODO(),\n\t\t"functionName",\n\t\tbem.FunctionGetParams{},\n\t)\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", functionResponse.Function)\n}\n',
      },
      python: {
        method: 'functions.retrieve',
        example:
  ...
  },
  {
    ...
    params: [
      'functionName: string;',
      "update_function: { type: 'extract'; displayName?: string; enableBoundingBoxes?: boolean; functionName?: string; outputSchema?: object; outputSchemaName?: string; preCount?: boolean; tabularChunkingEnabled?: boolean; tags?: string[]; } | { type: 'classify'; classifications?: { name: string; description?: string; functionID?: string; functionName?: string; isErrorFallback?: boolean; origin?: object; regex?: object; }[]; description?: string; displayName?: string; functionName?: string; nativeVisualInput?: boolean; tags?: string[]; } | { type: 'send'; destinationType?: 'webhook' | 's3' | 'google_drive'; displayName?: string; functionName?: string; googleDriveFolderId?: string; s3Bucket?: string; s3Prefix?: string; tags?: string[]; webhookSigningEnabled?: boolean; webhookUrl?: string; } | { type: 'split'; displayName?: string; functionName?: string; printPageSplitConfig?: { nextFunctionID?: string; nextFunctionName?: string; }; semanticPageSplitConfig?: { itemClasses?: object[]; }; splitType?: 'print_page' | 'semantic_page'; tags?: string[]; } | { type: 'join'; description?: string; displayName?: string; functionName?: string; joinType?: 'standard'; outputSchema?: object; outputSchemaName?: string; tags?: string[]; } | { type: 'payload_shaping'; displayName?: string; functionName?: string; shapingSchema?: string; tags?: string[]; } | { type: 'enrich'; config?: { steps: enrich_step[]; endpoints?: object[]; }; } | { type: 'parse'; displayName?: string; extraConfig?: { enableBoundingBoxes?: boolean; }; functionName?: string; parseConfig?: { defaultBucket?: string; extractEntities?: boolean; linkAcrossDocuments?: boolean; schema?: object; }; tags?: string[]; } | { type: 'render'; displayName?: string; functionName?: string; renderConfig?: { template: object; }; tags?: string[]; };",
    ],
    response:
      '{ function: object | object | object | object | object | object | object | object | object | object | object; }',
    perLanguage: {
  ...
  },
  {
    ...
    params: [
      'callIDs?: string[];',
      "callTypes?: 'workflow' | 'direct_function' | 'adhoc_function'[];",
      'endingBefore?: string;',
      'functionIDs?: string[];',
      'functionNames?: string[];',
      'limit?: number;',
  ...
  },
  {
    ...
    response:
      "{ call?: { callID: string; callType: 'workflow' | 'direct_function' | 'adhoc_function'; createdAt: string; errors: error_event[]; outputs: event[]; traceUrl: string; url: string; callReferenceID?: string; finishedAt?: string; functionID?: string; functionName?: string; functionType?: function_type; functionVersionNum?: number; input?: object; status?: 'pending' | 'running' | 'completed' | 'failed'; workflowID?: string; workflowName?: string; workflowVersionNum?: number; }; error?: string; }",
    markdown:
      "## retrieve\n\n`client.calls.retrieve(callID: string): { call?: call; error?: string; }`\n\n**get** `/v3/calls/{callID}`\n\n**Retrieve a workflow call by ID.**\n\nReturns the full call object including status, workflow details, terminal outputs,\nand terminal errors. `outputs` and `errors` are both populated once the call finishes —\nthey are not mutually exclusive (a partially-completed workflow may have both).\n\n## Status\n\n| Status | Description |\n|--------|-------------|\n| `pending` | Queued, not yet started |\n| `running` | Currently executing |\n| `completed` | All enclosed function calls finished without errors |\n| `failed` | One or more enclosed function calls produced an error event |\n\nPoll this endpoint or configure a webhook subscription to detect completion.\n\n### Parameters\n\n- `callID: string`\n\n### Returns\n\n- `{ call?: { callID: string; callType: 'workflow' | 'direct_function' | 'adhoc_function'; createdAt: string; errors: error_event[]; outputs: event[]; traceUrl: string; url: string; callReferenceID?: string; finishedAt?: string; functionID?: string; functionName?: string; functionType?: function_type; functionVersionNum?: number; input?: object; status?: 'pending' | 'running' | 'completed' | 'failed'; workflowID?: string; workflowName?: string; workflowVersionNum?: number; }; error?: string; }`\n\n  - `call?: { callID: string; callType: 'workflow' | 'direct_function' | 'adhoc_function'; createdAt: string; errors: { eventID: string; functionID: string; functionName: string; message: string; referenceID: string; callID?: string; createdAt?: string; eventType?: 'error'; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; kind?: string; metadata?: object; workflowID?: string; workflowName?: string; workflowVersionNum?: number; }[]; outputs: { eventID: string; functionID: string; functionName: string; itemCount: number; itemOffset: number; referenceID: string; transformedContent: object; avgConfidence?: number; callID?: string; correctedContent?: object | object | object[] | string | number | boolean; createdAt?: string; eventType?: 'transform'; fieldConfidences?: object; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; inputs?: object[]; inputType?: input_type; invalidProperties?: string[]; isRegression?: boolean; lastPublishErrorAt?: string; metadata?: object; metrics?: object; orderMatching?: boolean; pipelineID?: string; publishedAt?: string; s3URL?: string; transformationID?: string; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { eventID: string; functionID: string; functionName: string; itemCount: number; itemOffset: number; referenceID: string; transformedContent: object; avgConfidence?: number; callID?: string; correctedContent?: object | object | object[] | string | number | boolean; createdAt?: string; eventType?: 'extract'; fieldBoundingBoxes?: object; fieldConfidences?: object; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; inputs?: object[]; inputType?: input_type; invalidProperties?: string[]; metadata?: object; s3URL?: string; transformationID?: string; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { eventID: string; functionID: string; functionName: string; itemCount: number; itemOffset: number; referenceID: string; transformedContent: object; avgConfidence?: number; callID?: string; correctedContent?: object | object | object[] | string | number | boolean; createdAt?: string; eventType?: 'parse'; fieldBoundingBoxes?: object; fieldConfidences?: object; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; inputs?: object[]; inputType?: input_type; invalidProperties?: string[]; metadata?: object; s3URL?: string; transformationID?: string; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { eventID: string; functionID: string; functionName: string; invalidProperties: string[]; referenceID: string; transformedContent: object; avgConfidence?: number; callID?: string; createdAt?: string; eventType?: 'analyze'; fieldBoundingBoxes?: object; fieldConfidences?: object; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; metadata?: object; s3URL?: string; transformationID?: string; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { choice: string; eventID: string; functionID: string; functionName: string; referenceID: string; callID?: string; createdAt?: string; eventType?: 'route'; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; metadata?: object; s3URL?: string; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { choice: string; eventID: string; functionID: string; functionName: string; referenceID: string; callID?: string; createdAt?: string; eventType?: 'classify'; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; metadata?: object; s3URL?: string; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { eventID: string; functionID: string; functionName: string; outputType: 'print_page' | 'semantic_page'; printPageOutput: object; referenceID: string; semanticPageOutput: object; callID?: string; createdAt?: string; eventType?: 'split_collection'; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; metadata?: object; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { eventID: string; functionID: string; functionName: string; outputType: 'print_page' | 'semantic_page'; referenceID: string; callID?: string; createdAt?: string; eventType?: 'split_item'; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; metadata?: object; printPageOutput?: object; semanticPageOutput?: object; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | object | { eventID: string; functionID: string; functionName: string; invalidProperties: string[]; items: object[]; joinType: 'standard'; referenceID: string; transformedContent: object; avgConfidence?: number; callID?: string; createdAt?: string; eventType?: 'join'; fieldConfidences?: object; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; metadata?: object; transformationID?: string; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { enrichedContent: object; eventID: string; functionID: string; functionName: string; referenceID: string; callID?: string; createdAt?: string; eventType?: 'enrich'; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; groundTruth?: object; inboundEmail?: inbound_email_event; metadata?: object; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { eventID: string; functionID: string; functionName: string; referenceID: string; transformedContent: object; callID?: string; createdAt?: string; eventType?: 'payload_shaping'; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; metadata?: object; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { evaluationVersion: string; eventID: string; functionID: string; functionName: string; referenceID: string; result: object; status: 'success' | 'failed'; transformId: string; callID?: string; createdAt?: string; errorMessage?: string; eventType?: 'evaluation'; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; metadata?: object; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { collectionID: string; collectionName: string; eventID: string; operation: 'add' | 'update'; processedCount: number; referenceID: string; status: 'success' | 'failed'; collectionItemIDs?: string[]; createdAt?: string; errorMessage?: string; eventType?: 'collection_processing'; functionCallTryNumber?: number; inboundEmail?: inbound_email_event; metadata?: object; } | { deliveryStatus: 'success' | 'skip'; destinationType: send_destination_type; eventID: string; functionID: string; functionName: string; referenceID: string; callID?: string; createdAt?: string; deliveredContent?: object; eventType?: 'send'; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; googleDriveOutput?: object; inboundEmail?: inbound_email_event; metadata?: object; s3Output?: object; webhookOutput?: object; workflowID?: string; workflowName?: string; workflowVersionNum?: number; } | { docxRenderSeconds: number; eventID: string; functionID: string; functionName: string; outputDownloadURL: string; referenceID: string; validationSeconds: number; callID?: string; createdAt?: string; eventType?: 'render'; functionCallID?: string; functionCallTryNumber?: number; functionVersionNum?: number; inboundEmail?: inbound_email_event; metadata?: object; workflowID?: string; workflowName?: string; workflowVersionNum?: number; }[]; traceUrl: string; url: string; callReferenceID?: string; finishedAt?: string; functionID?: string; functionName?: string; functionType?: string; functionVersionNum?: number; input?: { batchFiles?: { inputs?: object[]; }; singleFile?: { inputType?: string; s3URL?: string; }; }; status?: 'pending' | 'running' | 'completed' | 'failed'; workflowID?: string; workflowName?: string; workflowVersionNum?: number; }`\n  - `error?: string`\n\n### Example\n\n```typescript\nimport Bem from 'bem-ai-sdk';\n\nconst client = new Bem();\n\nconst callGetResponse = await client.calls.retrieve('callID');\n\nconsole.log(callGetResponse);\n```",
    perLanguage: {
      cli: {
  ...
  },
  {
    ...
    response:
      'object | object | object | object | object | object | object | object | error_event | object | object | object | object | object | object | object',
    markdown:
  ...
  },
  {
    ...
    params: ['eventID: string;'],
    response:
      '{ output: object | object | object | object | object | object | object | object | error_event | object | object | object | object | object | object | object; }',
    markdown:
  ...
  },
  {
    ...
    params: [
      ...
      'endingBefore?: string;',
      'functionIDs?: string[];',
      'functionIDVersionNums?: string[];',
      'functionNames?: string[];',
      'functionNameVersionNums?: string[];',
      'limit?: number;',
      "sortOrder?: 'asc' | 'desc';",
      'startingAfter?: string;',
      'tags?: string[];',
  ...
  },
  {
    ...
    response:
      '{ copiedFunctions?: { sourceFunctionID: string; sourceFunctionName: string; sourceVersionNum: number; targetFunctionID: string; targetFunctionName: string; targetVersionNum: number; }[]; environment?: string; error?: string; workflow?: { id: string; connectors: object[]; createdAt: string; edges: workflow_edge_response[]; mainNodeName: string; name: string; nodes: workflow_node_response[]; restricted: boolean; updatedAt: string; versionNum: number; audit?: workflow_audit; displayName?: string; emailAddress?: string; tags?: string[]; }; }',
    markdown:
  ...
  },
  {
    ...
    response:
      "{ connectorErrors?: { code: string; message: string; operation: 'create' | 'update' | 'delete'; connectorID?: string; name?: string; }[]; error?: string; workflow?: { id: string; name: string; versionNum: number; }; }",
    markdown:
      "## delete\n\n`client.workflows.delete(workflowName: string): { connectorErrors?: workflow_connector_error[]; error?: string; workflow?: object; }`\n\n**delete** `/v3/workflows/{workflowName}`\n\n**Delete a workflow and every one of its versions.**\n\nPermanent. Running and queued calls against this workflow continue\nto completion against the version they captured at call time;\nsubsequent attempts to call the workflow return `404 Not Found`.\n\nFunctions referenced by the deleted workflow are not removed — they\nremain available to other workflows or for direct reference.\n\nAny connectors attached to the workflow are torn down first. Teardown is\nbest-effort: per-connector failures are reported in `connectorErrors` but\ndo not block the deletion, so check that array rather than relying on the\nstatus code alone.\n\n### Parameters\n\n- `workflowName: string`\n\n### Returns\n\n- `{ connectorErrors?: { code: string; message: string; operation: 'create' | 'update' | 'delete'; connectorID?: string; name?: string; }[]; error?: string; workflow?: { id: string; name: string; versionNum: number; }; }`\n\n  - `connectorErrors?: { code: string; message: string; operation: 'create' | 'update' | 'delete'; connectorID?: string; name?: string; }[]`\n  - `error?: string`\n  - `workflow?: { id: string; name: string; versionNum: number; }`\n\n### Example\n\n```typescript\nimport Bem from 'bem-ai-sdk';\n\nconst client = new Bem();\n\nconst workflow = await client.workflows.delete('workflowName');\n\nconsole.log(workflow);\n```",
    perLanguage: {
      cli: {
      ...
      csharp: {
        method: 'Workflows.Delete',
        example:
          'WorkflowDeleteParams parameters = new() { WorkflowName = "workflowName" };\n\nvar workflow = await client.Workflows.Delete(parameters);\n\nConsole.WriteLine(workflow);',
      },
      go: {
        method: 'client.Workflows.Delete',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/bem-team/bem-go-sdk"\n\t"github.com/bem-team/bem-go-sdk/option"\n)\n\nfunc main() {\n\tclient := bem.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t)\n\tworkflow, err := client.Workflows.Delete(context.TODO(), "workflowName")\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", workflow.ConnectorErrors)\n}\n',
      },
      python: {
        method: 'workflows.delete',
        example:
  ...
  },
  {
    ...
    params: ['workflowName: string;'],
    response:
      '{ error?: string; workflow?: { id: string; connectors: object[]; createdAt: string; edges: workflow_edge_response[]; mainNodeName: string; name: string; nodes: workflow_node_response[]; restricted: boolean; updatedAt: string; versionNum: number; audit?: workflow_audit; displayName?: string; emailAddress?: string; tags?: string[]; }; }',
    markdown:
  ...
  },
  {
    ...
    response:
      "{ connectorErrors?: { code: string; message: string; operation: 'create' | 'update' | 'delete'; connectorID?: string; name?: string; }[]; error?: string; workflow?: { id: string; connectors: object[]; createdAt: string; edges: workflow_edge_response[]; mainNodeName: string; name: string; nodes: workflow_node_response[]; restricted: boolean; updatedAt: string; versionNum: number; audit?: workflow_audit; displayName?: string; emailAddress?: string; tags?: string[]; }; }",
    markdown:
  ...
  },
  {
    ...
    response:
      "{ id: string; connectors: { connectorID: string; name: string; type: 'paragon'; paragon?: { configuration: object; integration: string; syncID: string; }; }[]; createdAt: string; edges: { destinationNodeName: string; sourceNodeName: string; destinationName?: string; metadata?: object; }[]; mainNodeName: string; name: string; nodes: { function: function_version_identifier; name: string; metadata?: object; }[]; restricted: boolean; updatedAt: string; versionNum: number; audit?: { versionCreatedBy?: user_action_summary; workflowCreatedBy?: user_action_summary; workflowLastUpdatedBy?: user_action_summary; }; displayName?: string; emailAddress?: string; tags?: string[]; }",
    markdown:
      "## list\n\n`client.workflows.versions.list(workflowName: string, endingBefore?: number, limit?: number, sortOrder?: 'asc' | 'desc', startingAfter?: number): { id: string; connectors: object[]; createdAt: string; edges: workflow_edge_response[]; mainNodeName: string; name: string; nodes: workflow_node_response[]; restricted: boolean; updatedAt: string; versionNum: number; audit?: workflow_audit; displayName?: string; emailAddress?: string; tags?: string[]; }`\n\n**get** `/v3/workflows/{workflowName}/versions`\n\n**List every version of a workflow.**\n\nVersions are immutable. Each row captures what the workflow looked\nlike between updates: graph topology, metadata, and timestamps.\nReturns newest-first by default. Cursor pagination via\n`startingAfter` / `endingBefore` over `versionNum`.\n\n### Parameters\n\n- `workflowName: string`\n\n- `endingBefore?: number`\n\n- `limit?: number`\n\n- `sortOrder?: 'asc' | 'desc'`\n\n- `startingAfter?: number`\n\n### Returns\n\n- `{ id: string; connectors: { connectorID: string; name: string; type: 'paragon'; paragon?: { configuration: object; integration: string; syncID: string; }; }[]; createdAt: string; edges: { destinationNodeName: string; sourceNodeName: string; destinationName?: string; metadata?: object; }[]; mainNodeName: string; name: string; nodes: { function: function_version_identifier; name: string; metadata?: object; }[]; restricted: boolean; updatedAt: string; versionNum: number; audit?: { versionCreatedBy?: user_action_summary; workflowCreatedBy?: user_action_summary; workflowLastUpdatedBy?: user_action_summary; }; displayName?: string; emailAddress?: string; tags?: string[]; }`\n  V3 read representation of a workflow version.\n\n  - `id: string`\n  - `connectors: { connectorID: string; name: string; type: 'paragon'; paragon?: { configuration: object; integration: string; syncID: string; }; }[]`\n  - `createdAt: string`\n  - `edges: { destinationNodeName: string; sourceNodeName: string; destinationName?: string; metadata?: object; }[]`\n  - `mainNodeName: string`\n  - `name: string`\n  - `nodes: { function: { id?: string; name?: string; versionNum?: number; }; name: string; metadata?: object; }[]`\n  - `restricted: boolean`\n  - `updatedAt: string`\n  - `versionNum: number`\n  - `audit?: { versionCreatedBy?: { createdAt: string; userActionID: string; apiKeyName?: string; emailAddress?: string; userEmail?: string; userID?: string; }; workflowCreatedBy?: { createdAt: string; userActionID: string; apiKeyName?: string; emailAddress?: string; userEmail?: string; userID?: string; }; workflowLastUpdatedBy?: { createdAt: string; userActionID: string; apiKeyName?: string; emailAddress?: string; userEmail?: string; userID?: string; }; }`\n  - `displayName?: string`\n  - `emailAddress?: string`\n  - `tags?: string[]`\n\n### Example\n\n```typescript\nimport Bem from 'bem-ai-sdk';\n\nconst client = new Bem();\n\n// Automatically fetches more pages as needed.\nfor await (const workflow of client.workflows.versions.list('workflowName')) {\n  console.log(workflow);\n}\n```",
    perLanguage: {
      cli: {
        method: 'versions list',
        example:
  ...
  },
  {
    ...
    perLanguage: {
      ...
      csharp: {
        method: 'Collections.List',
        example:
          'CollectionListParams parameters = new();\n\nvar page = await client.Collections.List(parameters);\nawait foreach (var item in page.Paginate())\n{\n    Console.WriteLine(item);\n}',
      },
      go: {
        method: 'client.Collections.List',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/bem-team/bem-go-sdk"\n\t"github.com/bem-team/bem-go-sdk/option"\n)\n\nfunc main() {\n\tclient := bem.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t)\n\tpage, err := client.Collections.List(context.TODO(), bem.CollectionListParams{})\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", page)\n}\n',
      },
      python: {
        method: 'collections.list',
        example:
          'import os\nfrom bem import Bem\n\nclient = Bem(\n    api_key=os.environ.get("BEM_API_KEY"),  # This is the default and can be omitted\n)\npage = client.collections.list()\npage = page.collections[0]\nprint(page.collection_id)',
      },
      typescript: {
        method: 'client.collections.list',
        example:
          "import Bem from 'bem-ai-sdk';\n\nconst client = new Bem({\n  apiKey: process.env['BEM_API_KEY'], // This is the default and can be omitted\n});\n\n// Automatically fetches more pages as needed.\nfor await (const collectionListResponse of client.collections.list()) {\n  console.log(collectionListResponse.collectionID);\n}",
      },
      http: {
  ...
  },
  {
    ...
    response:
      "{ functionName: string; functionVersionNum: number; perPair: { pairIndex: number; status: 'pending' | 'running' | 'completed' | 'failed'; callID?: string; errorMessage?: string; fieldResults?: { match: 'match' | 'mismatch' | 'missing' | 'extra'; path: string; actual?: object; delta?: number; expected?: object; similarity?: number; }[]; }[]; progress: { completed: number; failed: number; total: number; }; scoreRunID: string; status: 'pending' | 'initializing' | 'running' | 'completed' | 'error' | 'cancelled'; aggregate?: { extras: number; f1: number; matches: number; mismatches: number; missing: number; precision: number; recall: number; totalFieldsActual: number; totalFieldsExpected: number; }; }",
    markdown:
      "## retrieve\n\n`client.eval.score.retrieve(scoreRunID: string): { functionName: string; functionVersionNum: number; perPair: object[]; progress: object; scoreRunID: string; status: eval_score_run_status; aggregate?: object; }`\n\n**get** `/v3/eval/score/{scoreRunID}`\n\n**Get the status and per-pair results of a score run.**\n\nThe comparison happens here, not in the run: the function's output is compared\nagainst the expected value on every read, under the configuration supplied below.\nRe-reading the same run with different settings returns different metrics and\ncosts nothing — no model calls are repeated.\n\nComparison is exact and takes no configuration: a value matches the expected one\nor it is a miss. It is still redone on every read, so the numbers reflect the\nstored data as it is now.\n\nReturns `aggregate` once `status` reaches `completed` or `error`. `perPair` is\npopulated incrementally — each pair's `fieldResults` appears as its underlying\nfunction call terminates.\n\n### Parameters\n\n- `scoreRunID: string`\n\n### Returns\n\n- `{ functionName: string; functionVersionNum: number; perPair: { pairIndex: number; status: 'pending' | 'running' | 'completed' | 'failed'; callID?: string; errorMessage?: string; fieldResults?: { match: 'match' | 'mismatch' | 'missing' | 'extra'; path: string; actual?: object; delta?: number; expected?: object; similarity?: number; }[]; }[]; progress: { completed: number; failed: number; total: number; }; scoreRunID: string; status: 'pending' | 'initializing' | 'running' | 'completed' | 'error' | 'cancelled'; aggregate?: { extras: number; f1: number; matches: number; mismatches: number; missing: number; precision: number; recall: number; totalFieldsActual: number; totalFieldsExpected: number; }; }`\n  Full status payload returned by `GET /v3/eval/score/{scoreRunID}`.\n\nScoring takes no configuration: a value matches the expected one or it is a miss.\nThe comparison is still recomputed on every read from the stored JSON, so the\nnumbers reflect the data as it is now rather than as it was when the run executed.\n\n  - `functionName: string`\n  - `functionVersionNum: number`\n  - `perPair: { pairIndex: number; status: 'pending' | 'running' | 'completed' | 'failed'; callID?: string; errorMessage?: string; fieldResults?: { match: 'match' | 'mismatch' | 'missing' | 'extra'; path: string; actual?: object; delta?: number; expected?: object; similarity?: number; }[]; }[]`\n  - `progress: { completed: number; failed: number; total: number; }`\n  - `scoreRunID: string`\n  - `status: 'pending' | 'initializing' | 'running' | 'completed' | 'error' | 'cancelled'`\n  - `aggregate?: { extras: number; f1: number; matches: number; mismatches: number; missing: number; precision: number; recall: number; totalFieldsActual: number; totalFieldsExpected: number; }`\n\n### Example\n\n```typescript\nimport Bem from 'bem-ai-sdk';\n\nconst client = new Bem();\n\nconst evalScoreRun = await client.eval.score.retrieve('scoreRunID');\n\nconsole.log(evalScoreRun);\n```",
    perLanguage: {
      cli: {
      ...
      csharp: {
        ...
        example:
          'ScoreRetrieveParams parameters = new() { ScoreRunID = "scoreRunID" };\n\nvar evalScoreRun = await client.Eval.Score.Retrieve(parameters);\n\nConsole.WriteLine(evalScoreRun);',
      },
      go: {
        method: 'client.Eval.Score.Get',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/bem-team/bem-go-sdk"\n\t"github.com/bem-team/bem-go-sdk/option"\n)\n\nfunc main() {\n\tclient := bem.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t)\n\tevalScoreRun, err := client.Eval.Score.Get(context.TODO(), "scoreRunID")\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", evalScoreRun.FunctionName)\n}\n',
      },
      python: {
        method: 'eval.score.retrieve',
        example:
          'import os\nfrom bem import Bem\n\nclient = Bem(\n    api_key=os.environ.get("BEM_API_KEY"),  # This is the default and can be omitted\n)\neval_score_run = client.eval.score.retrieve(\n    "scoreRunID",\n)\nprint(eval_score_run.function_name)',
      },
      typescript: {
        method: 'client.eval.score.retrieve',
        example:
          "import Bem from 'bem-ai-sdk';\n\nconst client = new Bem({\n  apiKey: process.env['BEM_API_KEY'], // This is the default and can be omitted\n});\n\nconst evalScoreRun = await client.eval.score.retrieve('scoreRunID');\n\nconsole.log(evalScoreRun.functionName);",
      },
      http: {
  ...
  },
  {
    ...
    response:
      "{ functionName: string; functionVersionNum: number; perPair: { pairIndex: number; status: 'pending' | 'running' | 'completed' | 'failed'; callID?: string; errorMessage?: string; fieldResults?: { match: 'match' | 'mismatch' | 'missing' | 'extra'; path: string; actual?: object; delta?: number; expected?: object; similarity?: number; }[]; }[]; progress: { completed: number; failed: number; total: number; }; scoreRunID: string; status: 'pending' | 'initializing' | 'running' | 'completed' | 'error' | 'cancelled'; aggregate?: { extras: number; f1: number; matches: number; mismatches: number; missing: number; precision: number; recall: number; totalFieldsActual: number; totalFieldsExpected: number; }; }",
    markdown:
      "## cancel\n\n`client.eval.score.cancel(scoreRunID: string): { functionName: string; functionVersionNum: number; perPair: object[]; progress: object; scoreRunID: string; status: eval_score_run_status; aggregate?: object; }`\n\n**post** `/v3/eval/score/{scoreRunID}/cancel`\n\n**Cancel an in-flight score run.**\n\nTransitions the run to `cancelled`. Function calls already in flight\nare allowed to finish (best-effort cancellation via the job queue);\nresults from completed pairs may still appear in subsequent GETs.\n\n### Parameters\n\n- `scoreRunID: string`\n\n### Returns\n\n- `{ functionName: string; functionVersionNum: number; perPair: { pairIndex: number; status: 'pending' | 'running' | 'completed' | 'failed'; callID?: string; errorMessage?: string; fieldResults?: { match: 'match' | 'mismatch' | 'missing' | 'extra'; path: string; actual?: object; delta?: number; expected?: object; similarity?: number; }[]; }[]; progress: { completed: number; failed: number; total: number; }; scoreRunID: string; status: 'pending' | 'initializing' | 'running' | 'completed' | 'error' | 'cancelled'; aggregate?: { extras: number; f1: number; matches: number; mismatches: number; missing: number; precision: number; recall: number; totalFieldsActual: number; totalFieldsExpected: number; }; }`\n  Full status payload returned by `GET /v3/eval/score/{scoreRunID}`.\n\nScoring takes no configuration: a value matches the expected one or it is a miss.\nThe comparison is still recomputed on every read from the stored JSON, so the\nnumbers reflect the data as it is now rather than as it was when the run executed.\n\n  - `functionName: string`\n  - `functionVersionNum: number`\n  - `perPair: { pairIndex: number; status: 'pending' | 'running' | 'completed' | 'failed'; callID?: string; errorMessage?: string; fieldResults?: { match: 'match' | 'mismatch' | 'missing' | 'extra'; path: string; actual?: object; delta?: number; expected?: object; similarity?: number; }[]; }[]`\n  - `progress: { completed: number; failed: number; total: number; }`\n  - `scoreRunID: string`\n  - `status: 'pending' | 'initializing' | 'running' | 'completed' | 'error' | 'cancelled'`\n  - `aggregate?: { extras: number; f1: number; matches: number; mismatches: number; missing: number; precision: number; recall: number; totalFieldsActual: number; totalFieldsExpected: number; }`\n\n### Example\n\n```typescript\nimport Bem from 'bem-ai-sdk';\n\nconst client = new Bem();\n\nconst evalScoreRun = await client.eval.score.cancel('scoreRunID');\n\nconsole.log(evalScoreRun);\n```",
    perLanguage: {
      cli: {
        method: 'score cancel',
      ...
      csharp: {
        ...
        example:
          'ScoreCancelParams parameters = new() { ScoreRunID = "scoreRunID" };\n\nvar evalScoreRun = await client.Eval.Score.Cancel(parameters);\n\nConsole.WriteLine(evalScoreRun);',
      },
      go: {
        method: 'client.Eval.Score.Cancel',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/bem-team/bem-go-sdk"\n\t"github.com/bem-team/bem-go-sdk/option"\n)\n\nfunc main() {\n\tclient := bem.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t)\n\tevalScoreRun, err := client.Eval.Score.Cancel(context.TODO(), "scoreRunID")\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", evalScoreRun.FunctionName)\n}\n',
      },
      python: {
        method: 'eval.score.cancel',
        example:
          'import os\nfrom bem import Bem\n\nclient = Bem(\n    api_key=os.environ.get("BEM_API_KEY"),  # This is the default and can be omitted\n)\neval_score_run = client.eval.score.cancel(\n    "scoreRunID",\n)\nprint(eval_score_run.function_name)',
      },
      typescript: {
        method: 'client.eval.score.cancel',
        example:
  ...
  },
  {
    ...
    perLanguage: {
      ...
      csharp: {
        method: 'Views.List',
        example:
          'ViewListParams parameters = new();\n\nvar page = await client.Views.List(parameters);\nawait foreach (var item in page.Paginate())\n{\n    Console.WriteLine(item);\n}',
      },
      go: {
        method: 'client.Views.List',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/bem-team/bem-go-sdk"\n\t"github.com/bem-team/bem-go-sdk/option"\n)\n\nfunc main() {\n\tclient := bem.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t)\n\tpage, err := client.Views.List(context.TODO(), bem.ViewListParams{})\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", page)\n}\n',
      },
      python: {
        method: 'views.list',
        example:
  ...
  },
  {
    ...
    response:
      "{ aggregations: { function: 'count' | 'count_distinct' | 'sum' | 'average' | 'min' | 'max'; name: string; aggregateColumnName?: string; displayType?: 'table' | 'bar_chart' | 'pie_chart'; groupByColumnName?: string; }[]; columns: { displayOrderIndex: number; name: string; valueSchemaPath: string[]; }[]; currentVersionNum: number; filters: { columnName: string; filterType: string; number?: number; string?: string; }[]; functions: { id?: string; name?: string; }[]; name: string; viewID: string; description?: string; }",
    markdown:
      "## retrieve\n\n`client.views.retrieve(view_id: string): { aggregations: view_aggregation[]; columns: view_column[]; currentVersionNum: number; filters: view_filter[]; functions: function_identifier[]; name: string; viewID: string; description?: string; }`\n\n**get** `/v3/views/{view_id}`\n\n**Retrieve a view by ID.**\n\nReturns the view's current version. To inspect a historical version,\nfetch the list of versions on the View object and re-request with the\ndesired version pinned (versions are immutable once created).\n\n### Parameters\n\n- `view_id: string`\n\n### Returns\n\n- `{ aggregations: { function: 'count' | 'count_distinct' | 'sum' | 'average' | 'min' | 'max'; name: string; aggregateColumnName?: string; displayType?: 'table' | 'bar_chart' | 'pie_chart'; groupByColumnName?: string; }[]; columns: { displayOrderIndex: number; name: string; valueSchemaPath: string[]; }[]; currentVersionNum: number; filters: { columnName: string; filterType: string; number?: number; string?: string; }[]; functions: { id?: string; name?: string; }[]; name: string; viewID: string; description?: string; }`\n  A view is a table visualization of transformations that allows customers to have insight into their transformations\n\n  - `aggregations: { function: 'count' | 'count_distinct' | 'sum' | 'average' | 'min' | 'max'; name: string; aggregateColumnName?: string; displayType?: 'table' | 'bar_chart' | 'pie_chart'; groupByColumnName?: string; }[]`\n  - `columns: { displayOrderIndex: number; name: string; valueSchemaPath: string[]; }[]`\n  - `currentVersionNum: number`\n  - `filters: { columnName: string; filterType: string; number?: number; string?: string; }[]`\n  - `functions: { id?: string; name?: string; }[]`\n  - `name: string`\n  - `viewID: string`\n  - `description?: string`\n\n### Example\n\n```typescript\nimport Bem from 'bem-ai-sdk';\n\nconst client = new Bem();\n\nconst view = await client.views.retrieve('view_id');\n\nconsole.log(view);\n```",
    perLanguage: {
      cli: {
  ...
  },
  {
    ...
    perLanguage: {
      ...
      csharp: {
        method: 'Buckets.List',
        example:
          'BucketListParams parameters = new();\n\nvar page = await client.Buckets.List(parameters);\nawait foreach (var item in page.Paginate())\n{\n    Console.WriteLine(item);\n}',
      },
      go: {
        method: 'client.Buckets.List',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/bem-team/bem-go-sdk"\n\t"github.com/bem-team/bem-go-sdk/option"\n)\n\nfunc main() {\n\tclient := bem.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t)\n\tpage, err := client.Buckets.List(context.TODO(), bem.BucketListParams{})\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", page)\n}\n',
      },
      python: {
        method: 'buckets.list',
        example:
          'import os\nfrom bem import Bem\n\nclient = Bem(\n    api_key=os.environ.get("BEM_API_KEY"),  # This is the default and can be omitted\n)\npage = client.buckets.list()\npage = page.buckets[0]\nprint(page.bucket_id)',
      },
      typescript: {
        method: 'client.buckets.list',
        example:
  ...
  },
  {
    ...
    perLanguage: {
      ...
      csharp: {
        method: 'Buckets.Retrieve',
        example:
          'BucketRetrieveParams parameters = new() { BucketID = "bucketID" };\n\nvar bucketV3 = await client.Buckets.Retrieve(parameters);\n\nConsole.WriteLine(bucketV3);',
      },
      go: {
        method: 'client.Buckets.Get',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/bem-team/bem-go-sdk"\n\t"github.com/bem-team/bem-go-sdk/option"\n)\n\nfunc main() {\n\tclient := bem.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t)\n\tbucketV3, err := client.Buckets.Get(context.TODO(), "bucketID")\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", bucketV3.BucketID)\n}\n',
      },
      python: {
        method: 'buckets.retrieve',
        example:
          'import os\nfrom bem import Bem\n\nclient = Bem(\n    api_key=os.environ.get("BEM_API_KEY"),  # This is the default and can be omitted\n)\nbucket_v3 = client.buckets.retrieve(\n    "bucketID",\n)\nprint(bucket_v3.bucket_id)',
      },
      typescript: {
  ...
  },
  {
    ...
    perLanguage: {
      ...
      csharp: {
        method: 'Buckets.Update',
        example:
          'BucketUpdateParams parameters = new() { BucketID = "bucketID" };\n\nvar bucketV3 = await client.Buckets.Update(parameters);\n\nConsole.WriteLine(bucketV3);',
      },
      go: {
        method: 'client.Buckets.Update',
        example:
          'package main\n\nimport (\n\t"context"\n\t"fmt"\n\n\t"github.com/bem-team/bem-go-sdk"\n\t"github.com/bem-team/bem-go-sdk/option"\n)\n\nfunc main() {\n\tclient := bem.NewClient(\n\t\toption.WithAPIKey("My API Key"),\n\t)\n\tbucketV3, err := client.Buckets.Update(\n\t\tcontext.TODO(),\n\t\t"bucketID",\n\t\tbem.BucketUpdateParams{},\n\t)\n\tif err != nil {\n\t\tpanic(err.Error())\n\t}\n\tfmt.Printf("%+v\\n", bucketV3.BucketID)\n}\n',
      },
      python: {
        method: 'buckets.update',
        example:
          'import os\nfrom bem import Bem\n\nclient = Bem(\n    api_key=os.environ.get("BEM_API_KEY"),  # This is the default and can be omitted\n)\nbucket_v3 = client.buckets.update(\n    bucket_id="bucketID",\n)\nprint(bucket_v3.bucket_id)',
      },
      typescript: {
        method: 'client.buckets.update',
        example:
  ...
  },
  {
    ...
    params: ['entityIDs: string[];', "status: 'approved' | 'rejected';", 'bucket?: string;'],
    response:
  ...
  },
  {
    ...
    markdown:
      "## retrieve_seed_status\n\n`client.entities.retrieveSeedStatus(id: string): { createdCount: number; mergedCount: number; rejectedCount: number; seedJobID: string; status: 'pending' | 'processing' | 'completed' | 'failed'; totalRows: number; error?: string; results?: seed_row_result[]; }`\n\n**get** `/v3/entities/seed/{id}`\n\nGet Seed Job Status\n\n### Parameters\n\n- `id: string`\n\n### Returns\n\n- `{ createdCount: number; mergedCount: number; rejectedCount: number; seedJobID: string; status: 'pending' | 'processing' | 'completed' | 'failed'; totalRows: number; error?: string; results?: { canonical: string; outcome: 'created' | 'merged-with' | 'rejected'; entityID?: string; reason?: string; }[]; }`\n  `GET /v3/entities/seed/{id}` response.\n\n  - `createdCount: number`\n  - `mergedCount: number`\n  - `rejectedCount: number`\n  - `seedJobID: string`\n  - `status: 'pending' | 'processing' | 'completed' | 'failed'`\n  - `totalRows: number`\n  - `error?: string`\n  - `results?: { canonical: string; outcome: 'created' | 'merged-with' | 'rejected'; entityID?: string; reason?: string; }[]`\n\n### Example\n\n```typescript\nimport Bem from 'bem-ai-sdk';\n\nconst client = new Bem();\n\nconst response = await client.entities.retrieveSeedStatus('id');\n\nconsole.log(response);\n```",
    perLanguage: {
      cli: {
