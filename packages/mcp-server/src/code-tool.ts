...
import { ClientOptions } from 'bem-ai-sdk';

const prompt = `Runs JavaScript code to interact with the Bem API.
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    BEM_API_KEY: requireValue(
      readEnv('BEM_API_KEY') ?? client.apiKey,
      'set BEM_API_KEY environment variable or provide apiKey client option',
    ),
    BEM_BASE_URL: readEnv('BEM_BASE_URL') ?? client.baseURL ?? undefined,
  };
